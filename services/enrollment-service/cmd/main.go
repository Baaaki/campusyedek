...
func main() {
	...
	// Setup Gin router
	router := setupRouter(enrollmentHandler, cfg.Server.Environment, cfg.JWT.Secret)
...
func setupRouter(enrollmentHandler *handler.EnrollmentHandler, env string, jwtSecret string) *gin.Engine {
	if env == "production" {
	...
	{
		// Student routes
		// TODO: Add JWTAuth() middleware and role validation
		api.GET("/available-courses", enrollmentHandler.GetAvailableCourses)
		api.POST("/programs", enrollmentHandler.CreateEnrollmentProgram)
		api.GET("/my-enrollments", enrollmentHandler.GetMyEnrollments)
		api.GET("/latest-rejection", enrollmentHandler.GetLatestRejection)
		api.GET("/my-rejections", enrollmentHandler.GetMyRejections)

		// Advisor routes
		// TODO: Add JWTAuth() middleware and role validation
		api.GET("/advisor/pending-programs", enrollmentHandler.GetPendingProgramsByAdvisor)
		api.POST("/advisor/programs/:program_id/approve", enrollmentHandler.ApproveEnrollmentProgram)
		api.POST("/advisor/programs/:program_id/reject", enrollmentHandler.RejectEnrollmentProgram)
	}
